...
local function updateSounds(device, dt)
    local rpm = device.soundRPMSmoother:get(math.abs(device.outputAV1 * avToRPM), dt)
    local tq  = device.combustionTorque or device.lastOutputTorque or 0
    local pAbs = device.inductionPressure or 1.0

    -- grab max induction pressure (set during init from PressureThrottleMap)
    local maxP = device.maxInductionPressure or 1.0
    local hasFI = (maxP > 1.1)
    local pull = math.min(math.max(device.instantEngineLoad or 0, 0), 1)

    -- ===== TORQUE-NORMALIZED HUSH (quiet only near 0 torque) =====
    local rpmKey = math.floor(rpm)
    -- friction that Beam subtracts at runtime
    local fr = (device.frictionTorqueTable and device.frictionTorqueTable[rpmKey]) or 0

    -- positive cap from WOT row, apply density, subtract friction, then apply current FI limiting (matches runtime)
    local posCap = ((device.torqueCurve and device.torqueCurve[rpmKey]) or 1) * (device.intakeAirDensityCoef or 1)
    posCap = posCap - fr

    -- Apply boost-based torque scaling for FI engines.
    -- Scales WOT torque capacity (posCap) by actual vs. theoretical boost.
    -- Example: if we're at 1.3 bar of a 1.6 bar max, only ~81% of torque capacity counts.
    -- This keeps audio pull and hush timing consistent during spool-up.
    do
        local pRef = (device.peakPressureCurve and device.peakPressureCurve[rpmKey]) or maxP
        local pressureMult = (hasFI and pRef and pRef > 0.0001) and math.min((pAbs or 1.0) / pRef, 1) or 1
        posCap = posCap * pressureMult
    end
    if posCap < 10 then posCap = 10 end

    -- fractional width for each side, though negative functions on the max negative torque seen
    local kPos = device.soundConfiguration.exhaust.params.kPos or 0.90   -- loud by 90% of pos capacity
    local kNeg = device.soundConfiguration.exhaust.params.kNeg or 0.95   -- loud by 95% of neg capacity

    local refWidth
    local negCap = device.globalNegTorqueMax or 1
    if tq >= 0 then
        refWidth = math.max(kPos * posCap, 10)
    else
        refWidth = math.max(kNeg * (negCap), 10)
    end

    -- normalize by torque capacity
    local m
    m = math.min(math.abs(tq) / refWidth, 1)
    local function smooth01(x) return x * x * (3 - 2 * x) end
    local hushDepth = device.soundConfiguration.exhaust.params.hushDepth or 0.55
    local hush = 1.0 - hushDepth * (1 - smooth01(m))

    local baseVol = (rpm > 0.1) and device.engineVolumeCoef or 0
    local volOut  = baseVol * hush

    -- ===== MIX (two-bank crossfade) =====
    local minMixEng = device.soundMinLoadMix or 0
    local maxMixEng = device.soundMaxLoadMix or 1
    local engMix    = pull * (maxMixEng - minMixEng) + minMixEng

    local minMixExh = device.soundMinLoadMixExhaust or device.soundMinLoadMix or 0
    local maxMixExh = device.soundMaxLoadMixExhaust or device.soundMaxLoadMix or 1
    local exhMix    = pull * (maxMixExh - minMixExh) + minMixExh

    -- ===== APPLY TO FMOD =====
    if device.engineSoundID then
        local f0 = sounds.hzToFMODHz(rpm * device.soundConfiguration.engine.params.fundamentalFrequencyRPMCoef)
        obj:setEngineSound(device.engineSoundID, rpm, engMix, f0, volOut)
    end
    if device.engineSoundIDExhaust then
        local f0e = sounds.hzToFMODHz(rpm * device.soundConfiguration.exhaust.params.fundamentalFrequencyRPMCoef)
        obj:setEngineSound(device.engineSoundIDExhaust, rpm, exhMix, f0e, volOut)
    end
    ...
    -- ===== DEBUG LOG =====
    -- device._dbgAcc = (device._dbgAcc or 0) + dt
    -- if device._dbgAcc >= 0.25 then
    --     device._dbgAcc = 0
    --     log('I', 'sounddbg', string.format(
    --         "RPM=%d | TORQUE=%.1f | HUSH=%.2f | VOL=%.2f | LOAD=%.2f",
    --         rpm, tq, hush, volOut, device.instantEngineLoad
    --     ))
    -- end
end
    ...
    if not sounds.usesOldCustomSounds then
        ...
        if jbeamData.soundConfigExhaust then
            ...
            if soundConfig then
                ...
                --dump(params)
                -- allow extra tunables from the JBeam (flat or nested under "params")
                local extra = (soundConfig.params or soundConfig)
                if extra.kPos ~= nil then params.kPos = extra.kPos end
                if extra.kNeg ~= nil then params.kNeg = extra.kNeg end
                if extra.hushDepth ~= nil then params.hushDepth = extra.hushDepth end
...
local function new(jbeamData)
    ...
    do
        local maxP = 1.0
        if device.peakPressureCurve then
            for _, v in pairs(device.peakPressureCurve) do
            if type(v) == "number" and v > maxP then
                maxP = v
            end
            end
        end
        device.maxInductionPressure = maxP
        log("I", "engine_init", string.format("Max induction pressure set to %.2f bar", maxP))
    end

    -- === Build a dense per-RPM MIN torque curve across all throttle rows ===
    do
        local tmap = jbeamData and jbeamData.torque
        if tmap and #tmap >= 2 then
            local perRpmMin = {}         
            for i = 2, #tmap do
            local row = tmap[i]
            local rpm = math.max(0, math.floor(tonumber(row[2]) or 0))
            local tq  = tonumber(row[3]) or 0
            local cur = perRpmMin[rpm]
            if not cur or tq < cur then perRpmMin[rpm] = tq end
            end
            -- make sorted point list for createCurve
            local pts = {{0, 0}}
            for r, t in pairs(perRpmMin) do pts[#pts+1] = {r, t} end
            table.sort(pts, function(a,b) return a[1] < b[1] end)
            device.minTorqueCurve = createCurve(pts)   -- dense array-like curve
            log('I','engine_init', string.format("minTorqueCurve built (%d points).", #pts))
        else
            device.minTorqueCurve = nil
            log('W','engine_init', "No Automation torque table; minTorqueCurve not built.")
        end
    end

    device.frictionTorqueTable = rawFrictionTorque

    -- Global max engine-brake magnitude (positive number) including friction & 1.35 gain
    do
        local g = 0
        local maxRPM = device.maxRPM or 8000
        for r = 0, maxRPM do
            local negTable = device.minTorqueCurve and device.minTorqueCurve[r]
            if not negTable then
                local wot = (device.torqueCurve and device.torqueCurve[r]) or 0
                negTable = -0.35 * (wot * (device.intakeAirDensityCoef or 1))
            end
            local fr = (device.frictionTorqueTable and device.frictionTorqueTable[r]) or 0
            local cap = (negTable - fr)
            cap = -math.max(math.abs(cap) * 1.35, 10)
            local mag = -cap
            if mag > g then g = mag end
        end
        device.globalNegTorqueMax = g > 0 and g or 100
    end
